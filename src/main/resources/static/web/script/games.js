	...
	methods: {
		...
		joinGame: function(gameId) {
			...
			fetch(`http://localhost:8080/api/game/${gameId}/players`, {
				...
				.then((json) => {
					console.log(JSON.stringify(json.gpId));
		...
		getData: function() {
			fetch('http://localhost:8080/api/games', {
				...
				.then(function(json) {
					app.data = json;
					if (!app.data.player) {
						app.allGames = app.data.games;
						console.log(app.allGames);
					} else {
						app.allGames = app.data.games.filter((game) =>
							game.gamePlayers.some((gp) => gp.player.id === app.data.player.id)
						);
						app.showGames = app.data.games.filter((game) =>
							game.gamePlayers.every((gp) => gp.player.id !== app.data.player.id)
						);
						console.log(app.data.player.id);
